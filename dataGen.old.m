function dataGen(dataFile, maxNeigh, seedArr, jpath, relFile, outFile, patientCount, intGap, extGap, fixedSigma)
...
oldDataNonControl = dataNonControl(:, 1:patientCount);
...
save(outFile, 'dataAControl','dataANonControl', 'dataBControl', ...
    'dataBNonControl', 'oldDataNonControl', 'intGeneA','extGeneA', ...
...
for i =1:size(gRelatedGenes)
    ...
    if 1 == localGeneState
        ...
        lDataNonControl = mu + sigma * myrandn(patientCount);
    ...
    elseif 2 == localGeneState
       ...
       sigma = std(dataControl(localGene,:)); 
    	...
    	if 1 == sum(extGene == localGene)
       	...
       	lDataNonControl = mu2 + sigma * myrandn(patientCount);
